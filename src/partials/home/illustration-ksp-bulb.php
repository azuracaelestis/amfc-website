...
/* PORT THIS — KSP card 2 (創新/AI) lightbulb icon, INLINED rather than referenced as
   ...
   Placement corrected against the real Figma frame (427321480, supplied as a screenshot — the
   MCP Figma bridge only had an unrelated portfolio file open, not this project's file, so a
   screenshot was the working substitute): sparkle-a (large) sits to the LEFT of the glass, at
   roughly its upper-third height; sparkle-b (small) sits to the RIGHT, level with where the
   glass narrows into the base stripes. The first pass had guessed both sparkles onto the right
   side of the icon, which is why sparkle-a specifically needed to move — its translate now
   carries it outside this SVG's own tight viewBox (294–410), which is why overflow="visible" is
   set below: without it, the default SVG clip would cut sparkle-a off at the viewBox's left
   edge. Confirmed nothing else clips it: .amfc-philosophy__stat-card's own overflow:hidden only
   bounds the CARD, and this shift is nowhere near that edge. */
?>
<svg class="amfc-philosophy__stat-icon amfc-philosophy__stat-icon--lightbulb" viewBox="294 52 116 171" overflow="visible" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
<g class="bulb">
